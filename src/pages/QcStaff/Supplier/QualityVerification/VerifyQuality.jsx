...
const VerifyQuality = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (approvedQty + rejectedQty === 0) {
      alert("Tổng approvedQuantity + rejectedQuantity phải lớn hơn 0. Ít nhất một trong hai phải có giá trị > 0.");
      return;
    }
    ...
    // ✅ Validation: rejectedQuantity > 0 hoặc có notes nếu verificationResult = FAILED
    if (formData.verificationResult === "FAILED") {
      const notes = formData.notes?.toString().trim() || "";
      if (rejectedQty === 0 && !notes) {
        alert("Nếu kết quả kiểm tra là FAILED, rejectedQuantity phải lớn hơn 0 hoặc phải có ghi chú lý do.");
        return;
      }
    }
    ...
    dispatch(
      verifyQualityRequest({
        ...
        harvestBatchId: formData.harvestBatchId?.trim() || undefined, // Send undefined if empty
      })
    );
  };
