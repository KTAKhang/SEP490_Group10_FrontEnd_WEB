...
import { Package, ChevronLeft, ChevronRight, ShoppingBag, MapPin, Calendar, User, CreditCard, Search } from "lucide-react";
import {
  orderHistoryRequest,
  orderCancelRequest,
  clearOrderMessages,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
        ...
        <div className="flex flex-col gap-4 mb-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-sm font-medium text-gray-600 mr-1 hidden sm:inline">Status:</span>
            <button
            ...
            {FILTERABLE_STATUSES.map((status) => {
              ...
              return (
                <button
                  key={status.value}
                  onClick={() => {
                    setStatusFilters((prev) => (prev.includes(status.value) ? [] : [status.value]));
                    setPage(1);
                  }}
                  className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                    isActive ? "bg-green-600 text-white" : "bg-white border border-gray-300 text-gray-600 hover:border-green-400 hover:text-green-700"
                  }`}
                >
                  {status.label}
                </button>
          ...
          {historyLoading ? (
            ...
            <div className="py-20 text-center">
              ...
              <p className="text-gray-500 text-sm max-w-md mx-auto">
                When you place an order, it will show up here. Start shopping to see your history.
              </p>
            </div>
          ) : (
              ...
              {orders.map((order) => (
                ...
                >
                  ...
                  <div className="xl:w-56 flex-shrink-0 p-4 xl:py-5 xl:pr-0 flex flex-row xl:flex-col justify-between xl:justify-center gap-2 border-b xl:border-b-0 xl:border-r border-gray-100">
                    <span className="text-xs font-mono text-gray-500 truncate max-w-[140px] xl:max-w-none" title={order._id}>
                      {order._id}
                    </span>
                  ...
                  <div className="flex-1 min-w-0 p-4 xl:py-5 xl:px-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                      ...
                      <div className="flex items-center gap-2 text-gray-700 md:col-span-2 min-w-0">
                        <User className="w-4 h-4 text-gray-400 flex-shrink-0" />
                        <span className="truncate">{order.receiver_name} · {order.receiver_phone}</span>
                      </div>
                      <div className="flex items-start gap-2 text-gray-600 md:col-span-3 min-w-0">
                        <MapPin className="w-4 h-4 text-gray-400 flex-shrink-0 mt-0.5" />
                        <span className="line-clamp-2">{order.receiver_address}</span>
                      </div>
                    </div>
                  </div>
