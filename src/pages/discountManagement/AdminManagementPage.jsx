 ...
 * Discount Management Page for Admin
 * - Can approve/reject discount codes
 * - Can activate/deactivate approved discounts
 * - Can update discount codes (only if not used)
...
import {
    discountListRequest,
    discountApproveRequest,
    discountRejectRequest,
    discountActivateRequest,
    discountDeactivateRequest,
    discountUpdateAdminRequest,
...
const AdminManagementPage = () => {
    ...
    const loadDiscounts = useCallback(
        (query = {}) => {
            ...
            if (query.status && query.status !== "all") payload.status = query.status;
            if (query.isActive !== undefined && query.isActive !== "all") {
                payload.isActive = query.isActive === "true" || query.isActive === true;
            }
    ...
    useEffect(() => {
        loadDiscounts({ page: 1, status: statusFilter === "all" ? undefined : statusFilter, isActive: isActiveFilter });
    ...
    const handleRefresh = useCallback(() => {
        ...
        if (isActiveFilter !== "all") query.isActive = isActiveFilter;
        loadDiscounts(query);
        setTimeout(() => setLoadingRefresh(false), 450);
    }, [loadDiscounts, statusFilter, isActiveFilter, pageMeta.page, pageMeta.limit, sortBy, sortOrder]);
    ...
    const handleUpdateSubmit = (e) => {
        ...
        dispatch(discountUpdateAdminRequest(selectedDiscount._id, submitData));
    ...
    const ActiveBadge = ({ isActive }) => {
        return (
            <span
                className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-700"
                    }`}
            >
        ...
        <div className="min-h-screen bg-gray-50 p-6">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100">
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {list.map((discount) => (
                                        <tr key={discount._id} className="hover:bg-gray-50 transition-colors">
                                            <td className="px-6 py-4">
                                                ...
                                                {discount.createdBy && (
                                                    <div className="text-xs text-gray-400 mt-1">
                                                        Created by: {discount.createdBy?.user_name || "N/A"}
                                                    </div>
                                                )}
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="text-sm">
                                                    ...
                                                    <div className="text-gray-600">
                                                        Min: {discount.minOrderValue} vnđ | Max: {discount.maxDiscountAmount} vnđ
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                            ...
                                            <td className="px-6 py-4">
                                                <ActiveBadge isActive={discount.isActive} />
                                            </td>
                                            <td className="px-6 py-4">
                    ...
                    {/* Pagination */}
                    {!loading && !loadingRefresh && list.length > 0 && (
                    ...
                    <div className="bg-white rounded-2xl shadow-xl max-w-md w-full">
                        ...
                        <div className="p-6">
                            <p className="text-gray-700 mb-4">
                                Are you sure you want to reject discount code <strong>{selectedDiscount.code}</strong>?
                            </p>
